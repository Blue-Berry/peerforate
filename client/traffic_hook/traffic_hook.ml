...
let parse_ip_to_uint32 ip_str =
  ...
  | [a; b; c; d] -> Unsigned.UInt32.of_int (a lor (b lsl 8) lor (c lsl 16) lor (d lsl 24))
...
let setup ~interface ~target_ips ?debounce_ms () =
  ...
  let debounce_ns = match debounce_ms with
...
let start ~interface ~target_ips ?debounce_ms ~stop callback =
  ...
  RingBuffer.init events_map ~callback:handle_event (fun rb ->
    while not !stop do
      ignore (try RingBuffer.poll rb ~timeout:100 with _ -> stop := true; -1)
    done
  );
  cleanup ()
...
let start_eio ~sw ~interface ~target_ips ?debounce_ms callback =
  ...
  RingBuffer.init events_map ~callback:handle_event (fun rb ->
    try
      while true do
        Eio.Fiber.check ();  (* Check if switch was cancelled *)
