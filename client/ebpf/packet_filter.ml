...
let obj_path =
  ...
  let candidates = [
    Filename.concat exe_dir "packet_filter.bpf.o";  (* Next to executable *)
    "packet_filter.bpf.o";  (* Current directory *)
  ...
  | None ->
    ...
    Printf.eprintf "\nTry running from the project root or copy packet_filter.bpf.o next to the executable.\n";
    exit 1
let egress_prog_name = "packet_filter_egress"
let ingress_prog_name = "packet_filter_ingress"
...
let direction = uint8_t -: "direction"
...
let direction_str dir =
  if Unsigned.UInt8.to_int dir = 0 then "INGRESS" else "EGRESS"
...
let get_ifindex ifname =
  ...
  let if_nametoindex = 
    Foreign.foreign "if_nametoindex" (string @-> returning uint) 
  in
  let idx = if_nametoindex ifname in
  Unix.close fd;
  if Unsigned.UInt.to_int idx = 0 then
    failwith (Printf.sprintf "Interface %s not found" ifname)
  else
    Unsigned.UInt.to_int idx
...
let on_packet_detected event =
  ...
  let dir = direction_str (getf event direction) in
  ...
  Printf.printf "[%Ld] %s %s:%d -> %s:%d (%s, %d bytes)\n%!"
    timestamp dir src sport dst dport proto len;
...
let () =
  ...
  Printf.printf "Monitoring packets to/from %s on interface %s (ifindex %d)\n%!"
    target_ip_str ifname ifindex;
  ...
  let ingress_hook_created = ref false in
  ...
  (* Set up TC hook for ingress (incoming packets) *)
  let tc_hook_ingress = make C.Types.Bpf_tc.hook in
  setf tc_hook_ingress C.Types.Bpf_tc.ifindex ifindex;
  setf tc_hook_ingress C.Types.Bpf_tc.attach_point `INGRESS;
  let sz = Ctypes.sizeof C.Types.Bpf_tc.hook in
  setf tc_hook_ingress C.Types.Bpf_tc.sz (Unsigned.Size_t.of_int sz);

  let tc_opts_ingress = make C.Types.Bpf_tc.Opts.t in
  setf tc_opts_ingress C.Types.Bpf_tc.Opts.handle (Unsigned.UInt32.of_int 1);
  setf tc_opts_ingress C.Types.Bpf_tc.Opts.priority (Unsigned.UInt32.of_int 1);
  let sz = Ctypes.sizeof C.Types.Bpf_tc.Opts.t in
  setf tc_opts_ingress C.Types.Bpf_tc.Opts.sz (Unsigned.Size_t.of_int sz);
  ...
  let ingress_prog = bpf_object_find_program_by_name obj ingress_prog_name in
  ...
  if err <> 0 && err <> -17 (* EEXIST *) then begin
  ...
  if err <> 0 then begin
    ...
    if !egress_hook_created then
      ignore (C.Functions.bpf_tc_hook_destroy (addr tc_hook_egress));
    bpf_object_close obj;
    exit 1
  end;
  ...
  (* Create and attach ingress hook *)
  let err = C.Functions.bpf_tc_hook_create (addr tc_hook_ingress) in
  if err = 0 then ingress_hook_created := true;
  if err <> 0 && err <> -17 (* EEXIST *) then begin
    Printf.eprintf "Failed to create ingress TC hook: %d\n" err;
  ...
  setf tc_opts_ingress C.Types.Bpf_tc.Opts.prog_fd ingress_prog.fd;
  (* Set BPF_TC_F_REPLACE flag (value 1) to replace any existing filter *)
  setf tc_opts_ingress C.Types.Bpf_tc.Opts.flags (Unsigned.UInt32.of_int 1);
  let err = C.Functions.bpf_tc_attach (addr tc_hook_ingress) (addr tc_opts_ingress) in
  if err <> 0 then begin
    Printf.eprintf "Failed to attach ingress TC program: %d\n" err;
    ignore (C.Functions.bpf_tc_detach (addr tc_hook_egress) (addr tc_opts_egress));
    if !egress_hook_created then
      ignore (C.Functions.bpf_tc_hook_destroy (addr tc_hook_egress));
    if !ingress_hook_created then
      ignore (C.Functions.bpf_tc_hook_destroy (addr tc_hook_ingress));
    bpf_object_close obj;
    exit 1
  end;

  Printf.printf "Ingress filter attached\n%!";
  ...
  Printf.printf "%-20s %-8s %-21s %-21s %-6s %s\n%!" 
    "TIMESTAMP" "DIR" "SOURCE" "DEST" "PROTO" "LENGTH";
  Printf.printf "%s\n%!" (String.make 80 '-');
  ...
  RingBuffer.init events_map ~callback:handle_event (fun rb ->
    while !running do
      ...
      let _ = try RingBuffer.poll rb ~timeout:100 with _ -> running := false; -1 in
      ()
    done
  );
  ...
  Printf.printf "\nShutting down...\n%!";
  ignore (C.Functions.bpf_tc_detach (addr tc_hook_egress) (addr tc_opts_egress));
  ignore (C.Functions.bpf_tc_detach (addr tc_hook_ingress) (addr tc_opts_ingress));
  if !egress_hook_created then
    ignore (C.Functions.bpf_tc_hook_destroy (addr tc_hook_egress));
  if !ingress_hook_created then
    ignore (C.Functions.bpf_tc_hook_destroy (addr tc_hook_ingress));
