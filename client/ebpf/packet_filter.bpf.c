 ...
 * 
 * Supports both layer 2 (Ethernet) and layer 3 (raw IP) interfaces like WireGuard/tun.
 */
...
static __always_inline int process_ip_packet(struct __sk_buff *ctx, struct iphdr *ip, 
                                              void *data_end, __u8 direction, __u32 filter_ip)
{
...
int packet_filter_egress(struct __sk_buff *ctx)
{
     ...
     * For layer 3 interfaces like WireGuard/tun, data starts directly with IP header.
     * Check the IP version field (first 4 bits) to detect raw IP packets.
     */
    ip = data;
    ...
    return process_ip_packet(ctx, ip, data_end, 1, *filter_ip);
}

/* TC classifier hook for ingress traffic (packets being received) */
SEC("tc")
int packet_filter_ingress(struct __sk_buff *ctx)
{
    void *data_end = (void *)(__u64)ctx->data_end;
    void *data = (void *)(__u64)ctx->data;
    struct iphdr *ip;
    ...
    /* Check if source matches our target IP (incoming from target) */
    if (ip->saddr != *filter_ip)
        return TC_ACT_OK;

    return process_ip_packet(ctx, ip, data_end, 0, *filter_ip);
}
