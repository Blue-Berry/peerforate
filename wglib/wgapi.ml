...
module Key = struct
  ...
  let to_string (key : t) =
    let char_ptr = Ctypes.coerce (ptr uchar) (ptr char) (CArray.start key) in
  ...
  let of_string (s : string) : t =
    ...
    for i = 0 to len - 1 do
      CArray.set arr i (Unsigned.UChar.of_int (Char.code s.[i]))
    done;
    arr
  ;;
...
module Interface = struct
  ...
  let of_wg_device cdevice =
    ...
    let device =
      let port = getf cdevice Wg_device.listen_port |> Unsigned.UInt16.to_int in
      match port with
      | 0 -> device
    ...
    let device =
      let fwmark = getf cdevice Wg_device.fwmark |> Unsigned.UInt32.to_int in
      match fwmark with
      | 0 -> device
