...
let () =
  ...
  (match C.get_resource_record client Dns.Rr_map.Txt (name "key.vpn.local") with
   | Ok (_ttl, txt_set) ->
  ...
  (match C.gethostbyname client (name "google.com") with
   | Ok ipv4 ->
  ...
  match C.getaddrinfo client Dns.Rr_map.Txt (name "test.vpn.local") with
  | Ok (_ttl, txt_set) ->
    let records = Dns.Rr_map.Txt_set.elements txt_set in
    Logs.info (fun m -> m "Found %d TXT record(s)" (List.length records));
    List.iter (fun txt -> print_endline ("TXT: " ^ txt)) records
